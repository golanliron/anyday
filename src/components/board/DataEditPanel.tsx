  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <input value={newItemName} onChange={e => setNewItemName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAddItem()} placeholder={"+ \u05e4\u05e8\u05d9\u05d8 \u05d7\u05d3\u05e9..."} style={{ flex: 1, background: "#FFF", border: "1.5px solid " + hexToRgba(pc, 0.15), borderRadius: 8, padding: "7px 12px", fontSize: 13, outline: "none", color: "#2D2252" }} />
      ...
      <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={"\u05d7\u05d9\u05e4\u05d5\u05e9..."} style={{ width: "100%", background: "#F9F7FF", border: "1px solid " + hexToRgba(pc, 0.1), borderRadius: 8, padding: "7px 12px", fontSize: 12, outline: "none", marginBottom: 8, color: "#2D2252" }} />
      ...
      <div style={{ overflowX: "auto", maxHeight: "calc(100vh - 300px)", overflowY: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12 }}>
          <thead><tr>
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: hexToRgba(pc, 0.06), padding: "8px 10px", textAlign: "right", fontWeight: 700, color: "#2D2252", fontSize: 11, borderBottom: "2px solid " + pc, whiteSpace: "nowrap", minWidth: 120 }}>{"\u05e9\u05dd"}</th>
            {editableCols.map(col => (<th key={col.id} style={{ position: "sticky", top: 0, zIndex: 2, background: hexToRgba(pc, 0.06), padding: "8px 6px", textAlign: "right", fontWeight: 700, color: "#2D2252", fontSize: 11, borderBottom: "2px solid " + pc, whiteSpace: "nowrap", minWidth: 80 }}>{col.title}</th>))}
