    ...
    if (this.state.error) {
      return (
        <div style={{ padding: 20, color: "#E17055", textAlign: "center" }}>
          ...
          <button onClick={() => this.setState({ error: null })} style={{ marginTop: 12, background: "#0035FF", color: "#FFF", border: "none", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 13 }}>נסה שוב</button>
        </div>
      );
    }
...
function DataEditPanelInner({ board, items, apiToken, boardId, pc = "#0035FF", ac = "#FFEF00" }: {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <input value={newItemName} onChange={e => setNewItemName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAddItem()} placeholder={"+ \u05e4\u05e8\u05d9\u05d8 \u05d7\u05d3\u05e9..."} style={{ flex: 1, background: "#FFF", border: "1.5px solid " + hexToRgba(pc, 0.15), borderRadius: 8, padding: "7px 12px", fontSize: 13, outline: "none", color: "#0035FF" }} />
      ...
      <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder={"\u05d7\u05d9\u05e4\u05d5\u05e9..."} style={{ width: "100%", background: "#F8F9FF", border: "1px solid " + hexToRgba(pc, 0.1), borderRadius: 8, padding: "7px 12px", fontSize: 12, outline: "none", marginBottom: 8, color: "#0035FF" }} />
      ...
      <div style={{ overflowX: "auto", maxHeight: "calc(100vh - 300px)", overflowY: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12 }}>
          <thead><tr>
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: hexToRgba(pc, 0.06), padding: "8px 10px", textAlign: "right", fontWeight: 700, color: "#0035FF", fontSize: 11, borderBottom: "2px solid " + pc, whiteSpace: "nowrap", minWidth: 120 }}>{"\u05e9\u05dd"}</th>
            {editableCols.map(col => (<th key={col.id} style={{ position: "sticky", top: 0, zIndex: 2, background: hexToRgba(pc, 0.06), padding: "8px 6px", textAlign: "right", fontWeight: 700, color: "#0035FF", fontSize: 11, borderBottom: "2px solid " + pc, whiteSpace: "nowrap", minWidth: 80 }}>{col.title}</th>))}
            <th style={{ position: "sticky", top: 0, zIndex: 2, background: hexToRgba(pc, 0.06), padding: "8px 4px", borderBottom: "2px solid " + pc, width: 30 }} />
          </tr></thead>
          <tbody>
            {filteredItems.slice(0, 100).map((item, idx) => (<tr key={item.id} style={{ background: idx % 2 === 0 ? "#FFF" : hexToRgba(pc, 0.02) }}>
              <td style={{ padding: "8px 10px", fontWeight: 600, color: "#0035FF", borderBottom: "1px solid " + hexToRgba(pc, 0.06), maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", borderRight: "3px solid " + pc }}>{item.name}</td>
              {editableCols.map(col => { const cv = (item.column_values || []).find(v => v.id === col.id); const value = cv?.text || ""; const isEditing = editingCell?.itemId === item.id && editingCell?.colId === col.id; const wasSaved = savedCells.has(item.id + "-" + col.id); const isStatus = col.type === "color"; return (<td key={col.id} style={{ padding: "4px 6px", borderBottom: "1px solid " + hexToRgba(pc, 0.06), position: "relative" }}>{isEditing ? (isStatus ? (<div style={{ display: "flex", flexDirection: "column", gap: 2, position: "absolute", top: 0, right: 0, zIndex: 10, background: "#FFF", border: "1.5px solid " + pc, borderRadius: 8, padding: 6, minWidth: 120, boxShadow: "0 4px 16px rgba(0,0,0,0.12)" }}>{(statusOptions[col.id] || []).map(opt => (<button key={opt} onClick={() => handleSave(item.id, col.id, opt)} disabled={saving} style={{ background: opt === value ? pc : "transparent", color: opt === value ? "#FFF" : "#0035FF", border: "none", borderRadius: 4, padding: "4px 8px", fontSize: 11, fontWeight: 600, cursor: saving ? "wait" : "pointer", textAlign: "right" }}>{opt}</button>))}<button onClick={() => setEditingCell(null)} style={{ background: "none", border: "none", fontSize: 10, color: "#E17055", cursor: "pointer", fontWeight: 600 }}>{"\u05d1\u05d9\u05d8\u05d5\u05dc"}</button></div>) : (<input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => { if (e.key === "Enter") handleSave(item.id, col.id, editValue); if (e.key === "Escape") setEditingCell(null); }} onBlur={() => { if (editValue !== value) handleSave(item.id, col.id, editValue); else setEditingCell(null); }} style={{ width: "100%", background: "#FFF", border: "1.5px solid " + pc, borderRadius: 4, padding: "4px 6px", fontSize: 12, outline: "none", color: "#0035FF" }} />)) : (<div onClick={() => { setEditingCell({ itemId: item.id, colId: col.id }); setEditValue(value); }} style={{ cursor: "pointer", padding: "4px 6px", borderRadius: 4, background: isStatus && value ? hexToRgba(pc, 0.08) : "transparent", border: wasSaved ? "1.5px solid #00B894" : "1.5px solid transparent", color: value ? "#0035FF" : "#CCC", fontWeight: isStatus ? 600 : 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 110, fontSize: 12 }}>{value || "\u2014"}</div>)}</td>); })}
